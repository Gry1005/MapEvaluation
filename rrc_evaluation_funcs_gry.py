...
def print_help():
    sys.stdout.write(
        'Usage: python %s.py -g=<gtFolder> -s=<submFolder> [-o=<outputFolder> -p=<jsonParams>]' % sys.argv[0])
    sys.exit(2)
...
def load_folder(file, fileNameRegExp='', allEntries=False):
    """
    Returns an array with the contents (filtered by fileNameRegExp) of a ZIP file.
    The key's are the names or the file or the capturing group definied in the fileNameRegExp
    allEntries validates that all entries in the ZIP file pass the fileNameRegExp
    ...
    try:
        ...
        raise Exception('Error loading the directory archive')
    ...
    for name in file_list:
        addFile = True
        # keyName = name
        keyName = os.path.basename(name)
        # if fileNameRegExp!="":
        #    m = re.match(fileNameRegExp,keyName)
        #    if m == None:
        #        addFile = False
        #    else:
        #        if len(m.groups())>0:
        ...
        if addFile:
            ...
            pairs.append([keyName, file_contents])  # file name : file_path
        else:
            if allEntries:
                raise Exception('ZIP entry not valid: %s' % name)
...
def validate_lines_in_file(fileName, file_contents, CRLF=True, LTRB=2, withTranscription=False, withConfidence=False,
...
def validate_tl_line(line, LTRB=2, withTranscription=False, withConfidence=True, imWidth=0, imHeight=0):
    """
    Validate the format of the line. If the line is not valid an exception will be raised.
    If maxWidth and maxHeight are specified, all points must be inside the imgage bounds.
    Posible values are:
    LTRB=1: xmin,ymin,xmax,ymax[,confidence][,transcription]
    LTRB=2: x1,y1,x2,y2,x3,y3,x4,y4[,confidence][,transcription]
    LTRB=3: x1,y1,x2,y2,x3,y3,x4,y4,x5,y5,...[,confidence][,transcription]
    """
    get_tl_line_values(line, LTRB, withTranscription, withConfidence, imWidth, imHeight)


def get_tl_line_values(line, LTRB=2, withTranscription=False, withConfidence=False, imWidth=0, imHeight=0):
    """
    Validate the format of the line. If the line is not valid an exception will be raised.
    If maxWidth and maxHeight are specified, all points must be inside the imgage bounds.
    Posible values are:
    ...
    if LTRB == 1:
        ...
        points = validate_and_convert_clockwise_points(points)
        ...
        assert withConfidence == False, "withConfidence should be False"
        ...
        #print('withTranscription:',withTranscription)
        #print('len(m):',len(m))

        #!!!
        #withTranscription=True

        if withTranscription == False and len(m) % 2 != 0:
            raise Exception("Format incorrect. Should be: x1,y1,x2,y2,x3,y3,x4,y4,...")
        elif withTranscription == True and len(m) % 2 != 1:
            raise Exception("Format incorrect. Should be: x1,y1,x2,y2,x3,y3,x4,y4,..., transcription")

        if len(m) % 2 == 0:
            points = [float(a) for a in m]
        else:
            points = [float(a) for a in m[:-1]]
        ...
        if (imWidth > 0 and imHeight > 0):
            for i in range(len(m) / 2):
                validate_point_inside_bounds(points[2 * i], points[2 * i + 1], imWidth, imHeight)
...
def validate_and_convert_clockwise_points(points):
    """
    Validates that the points are in clockwise order. If not, convert to clockwise order
    """
    edge = []
    for i in range(len(points) // 2):
        edge.append((int(points[(i + 1) * 2 % len(points)]) - int(points[i * 2])) * (
    ...
    if sum(edge) > 0:
        ...
        for i in range(0, len(points) // 2)[::-1]:
            ret_points.append(points[2 * i])
            ret_points.append(points[2 * i + 1])
        return ret_points
    else:
        return points
...
def main_evaluation(p, default_evaluation_params_fn, validate_data_fn, evaluate_method_fn, pa, show_result=True,
                    per_sample=True):
    """
    ...
    try:
        ...
        #!!!运行到这里就中断了--script_grt.py: evaluate_method
        evalData = evaluate_method_fn(p['g'], p['s'], evalParams)
        #print('Here!!')
        resDict.update(evalData)
